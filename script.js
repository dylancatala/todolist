...
getSaveButton.addEventListener("click", () => {
    if (getTaskNameInput.value !== "") {
        addTask(getTaskNameInput.value)
    }
});


function addTask(name) {
    taskList.push({id: uuidv4(), name: name})
...
function validateTask(taskItem) {
    const getTaskList = JSON.parse(localStorage.getItem('TaskList')) ?? []
    const getValidateTask = JSON.parse(localStorage.getItem('ValidateTask')) ?? []

    const findItem = getTaskList.find((item) => {
        return item.id === taskItem.id
    })

    getValidateTask.push(findItem)

    localStorage.setItem('ValidateTask', JSON.stringify(getValidateTask))
